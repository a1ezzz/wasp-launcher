...
class WConfigHostApp(WSyncHostApp):
	...
	- configuration file from WConfigHostApp.__environment_file_var__ path
	- configuration files that are resides in WConfigHostApp.__environment_dir_var__ (files are ordered by name)
	"""
	...
	def read_config(cls):
		...
		override it by a file given via :attr:`WConfigHostApp.__environment_file_var__` environment variable.
		After that configuration files are applied from :attr:`WConfigHostApp.__environment_dir_var__`
